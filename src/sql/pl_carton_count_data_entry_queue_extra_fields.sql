...
distinct
documentable_id
, max(prep_documents.created_ts) as created_at
...
, hand_off as (
  ...
  where
  body ilike '%#tablevalidate%%Data Services%'
)
, pending_note as (
  select
  messageable_id
  ...
  and datediff(days,created_ts,current_timestamp()) < 2
  )
...
, customs_invoice as (
  ...
  select
    ...
    , shipment_involved_parties.shipment_id


  from
  entities.shipment_involved_parties
...
, case when ((pl_drop_off.messageable_id is not null) and (pl_uploads.created_at < pl_notes.created_at)) then 'Complete' else 'No' end as pl_count_digitized
...
left join pl_notes
on pl_notes.messageable_id = bi_shipments.shipment_id
left join hand_off
on hand_off.messageable_id  = bi_shipments.shipment_id
left join pending_note on bi_shipments.shipment_id = pending_note.messageable_id and pending_note.rank = 1  and pending_note.created_ts > scheduled_origin_pickup_date
...
left join flexport_import_broker on flexport_import_broker.shipment_id = legacy.bi_shipments.shipment_id  and lower(import_customs_agent_company_entity_name) like '%flexport%'



where
(customs_invoiced = 1 or uses_customs_service = 'TRUE')
and pl_count_digitized like 'No'
and supply_shipments.destination_country_name not in ('Canada', 'United States')
and flexport_import_broker.shipment_id is null
