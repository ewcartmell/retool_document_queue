...
window.build_or_assign = function build_or_assign(arr = []) {
  arr.forEach(function (item, index) {
    ...
    if(!(doc == null) && doc.hasOwnProperty('ARRIVAL_DATE')) {
      item.ARRIVAL_DATE = (item.ARRIVAL_DATE instanceof Date) ? item.ARRIVAL_DATE : new Date(doc.ARRIVAL_DATE)
    }
    if(!(doc == null) && doc.hasOwnProperty('DUE_DATE')) {
      item.DUE_DATE = (item.DUE_DATE instanceof Date) ? item.DUE_DATE : new Date(doc.DUE_DATE)
    }
