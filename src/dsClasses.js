...
window.build_or_assign = function build_or_assign(arr = []) {
  arr.forEach(function (item, index) {
    ...
    if(!(doc == null) && doc.hasOwnProperty('ARRIVAL_DATE')) {
      item.ARRIVAL_DATE = (typeof item.ARRIVAL_DATE == "object") ? item.ARRIVAL_DATE : new Date(item.ARRIVAL_DATE)
    }
    if(!(doc == null) && doc.hasOwnProperty('DUE_DATE')) {
      item.DUE_DATE = (typeof item.DUE_DATE == "object") ? item.DUE_DATE : new Date(item.DUE_DATE)
    }
